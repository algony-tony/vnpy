    ...
    try:
        ...
        for k in dir(module):
            if 'Strategy' in k:
                ...
                STRATEGY_CLASS[k] = v
                tlog.debug('Imported Strategy Class {stcl}'.format(stcl=k))
    except:
        tlog.info('Failed to import strategy file %s:' %moduleName)
        tlog.info('{bt}'.format(bt=traceback.print_exc()))


# 遍历strategy目录下的文件
path = os.path.abspath(os.path.dirname(__file__))
for root, subdirs, files in os.walk(path):
    tlog.debug('Import strategies under {tld}'.format(tld=path))
    for name in files:
...
for root, subdirs, files in os.walk(workingPath):
    tlog.debug('Import strategies under {tld}'.format(tld=workingPath))
    for name in files:
        # 只有文件名中包含strategy且以.py结尾的文件，才是策略文件
        if 'strategy' in name and name[-3:] == '.py' and '/' not in name and '\\' not in name:
            # 模块名称无需前缀
